...
Based on my analysis of the headers and the current `coreaudio.pxd` and `coreaudio.pyx` files, here's a comprehensive summary of the APIs that are **not yet wrapped**:

## Currently Wrapped APIs (in coreaudio.pxd/coreaudio.pyx)

**CoreAudio:**

- AudioHardware (device management, properties, I/O procs)
- CoreAudioTypes (basic types, time stamps, SMPTE time)
- AudioQueue (playback/recording)
- AudioFile (file I/O)
- AudioComponent/AudioUnit (component discovery and management)
- AudioOutputUnit (basic output control)

**CoreFoundation:**

- Basic types (CFBase.h)
- CFURL, CFString (limited)

## Major Unwrapped APIs by Category

### 1. **AudioToolbox - Audio Processing & Conversion**

- **AudioConverter** - Format conversion, sample rate conversion, encoding/decoding
- **AudioFormat** - Audio format utilities, channel layout, panning, balance/fade
- **AudioFileStream** - Streaming audio file parsing
- **ExtendedAudioFile** - High-level audio file access with format conversion
- **AudioServices** - System sound services, UI sound effects
- **AudioSession** (iOS) - Audio session management (deprecated but still used)

### 2. **AudioToolbox - Music & MIDI**

- **MusicDevice** - Software synthesizer/instrument APIs
- **MusicPlayer** - Music sequencing and playback
- **AUGraph** - Audio unit graph management (deprecated but still functional)
- **AudioUnitUtilities** - Audio unit utility functions
- **AudioWorkInterval** - High-priority audio work intervals

### 3. **AudioToolbox - Advanced Features**

- **AudioCodec** - Audio codec management
- **AudioFileComponent** - Audio file component system
- **AudioHardwareService** - Hardware service APIs
- **CoreAudioClock** - Audio clock services
- **DefaultAudioOutput** - Default output management
- **CAShow** - Debug/logging utilities
- **CAFFile** - Core Audio Format file structures

### 4. **CoreFoundation - Data Structures & Utilities**

- **CFArray** - Array data structure
- **CFDictionary** - Dictionary/hash table data structure
- **CFData** - Data buffer management
- **CFNumber** - Number handling
- **CFSet** - Set data structure
- **CFRunLoop** - Event loop management
- **CFNotificationCenter** - Notification system
- **CFPreferences** - User preferences
- **CFPropertyList** - Property list handling
- **CFBundle** - Bundle management
- **CFError** - Error handling
- **CFString** (extended) - String utilities beyond basic types
- **CFURL** (extended) - URL utilities beyond basic creation
- **CFDate/CFTimeZone** - Date/time handling
- **CFSocket/CFStream** - Network I/O
- **CFUUID** - UUID generation
- **CFXMLParser** - XML parsing

### 5. **CoreMIDI - MIDI Support**

- **CoreMIDI** - Complete MIDI framework
- **MIDIServices** - MIDI device and connection management
- **MIDISetup** - MIDI configuration
- **MIDIThruConnection** - MIDI routing
- **MIDIDriver** - MIDI driver interface
- **MIDIMessages** - MIDI message handling
- **MIDIBluetoothConnection** - Bluetooth MIDI
- **MIDINetworkSession** - Network MIDI
- **MIDICapabilityInquiry** - MIDI device capabilities

### 6. **CoreAudioKit - UI Components**

- **CoreAudioKit** - Audio unit UI components
- **AUGenericView** - Generic audio unit UI
- **AUPannerView** - Panner UI
- **AUViewController** - Audio unit view controllers
- **CABTLEMIDIWindowController** - Bluetooth MIDI UI
- **CAInterDeviceAudioViewController** - Inter-device audio UI

### 7. **CoreAudio - Advanced Hardware**

- **AudioDriverPlugIn** - Audio driver plugin system
- **AudioServerPlugIn** - Audio server plugin system
- **AudioHardwareTapping** - Audio hardware tapping
- **CATapDescription** - Audio tap descriptions
...
1. **AudioConverter** - Essential for format conversion
2. **ExtendedAudioFile** - High-level file I/O with conversion
3. **AudioFormat** - Channel layout and format utilities
4. **CoreFoundation data structures** (CFArray, CFDictionary, CFData)
5. **CoreMIDI** - Complete MIDI support

**Medium Priority:**

1. **AudioServices** - System sounds
2. **MusicDevice/MusicPlayer** - Music synthesis
3. **AudioFileStream** - Streaming audio
4. **AUGraph** - Audio unit graphs (though deprecated)

**Lower Priority:**

1. **CoreAudioKit** - UI components
2. **AudioSession** - iOS audio session (deprecated)
3. **Advanced CoreAudio** - Driver plugins, hardware tapping

[1 tool called]

The analysis shows that while you have good coverage of the core audio playback and basic file I/O APIs, there are many significant APIs not yet wrapped, particularly around audio format conversion, MIDI support, CoreFoundation data structures, and advanced audio processing features. The most impactful additions would be **AudioConverter**, **ExtendedAudioFile**, **CoreFoundation collections**, and **CoreMIDI** support.
