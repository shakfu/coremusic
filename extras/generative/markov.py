...
class MarkovChain:
    ...
    def __init__(self, config: ChainConfig | None = None):
        ...
        self._duration_chain: MarkovChain | None = None
        self._velocity_chain: MarkovChain | None = None

        # Rhythm chain (IOI)
        self._rhythm_chain: MarkovChain | None = None
    ...
    def train(self, notes: Sequence[int | NoteData]) -> None:
    ...
    def _recompute_probabilities(self) -> None:
        ...
        for _state, edges in self._transitions.items():
    ...
    def sample_rhythm(self, history: list[float] | None = None) -> float:
    ...
    def _build_state_key(self, history: list[int] | None) -> StateKey:
        """Build state key from history."""
...
class MIDIMarkovAnalyzer:
    ...
    def analyze_all_tracks(
        ...
        for _i, track in enumerate(sequence.tracks):
...
class MIDIMarkovGenerator:
    ...
    def __init__(
        self,
        chain: MarkovChain,
        rhythm_generator: Any | None = None,
    ):
...
def merge_chains(
    ...
    for chain, weight in zip(chains, weights, strict=True):
