...
- **Real-time audio output stream** - `capi.AudioOutputStreamImpl` installs a Cython render callback on a Default Output unit that pulls each buffer from a Python generator (interleaved float32; best-effort real time, suitable for tone/synthesis generation). `coremusic.audio.streaming.AudioOutputStream` is wired to it, replacing the previous `NotImplementedError` stub; it accepts bytes or NumPy float generators (mono is broadcast to the requested channel count) and is safe against generator exceptions. Real-time input capture (`AudioInputStream`) and `create_loopback()` remain to be implemented.
