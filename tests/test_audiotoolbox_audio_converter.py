...
class TestAudioConverterAPI:
    ...
    def dest_format_mono(self):
        ...
        return {'sample_rate': 44100.0, 'format_id': capi.
            get_audio_format_linear_pcm(), 'format_flags': 12,
            'bytes_per_packet': 2, 'frames_per_packet': 1,
            'bytes_per_frame': 2, 'channels_per_frame': 1,
            'bits_per_channel': 16, 'reserved': 0}

    @pytest.fixture
    def dest_format_48k(self):
        """Fixture providing destination format (48kHz, stereo, 16-bit)"""
        return {'sample_rate': 48000.0, 'format_id': capi.
            get_audio_format_linear_pcm(), 'format_flags': 12,
            'bytes_per_packet': 4, 'frames_per_packet': 1,
            'bytes_per_frame': 4, 'channels_per_frame': 2,
            'bits_per_channel': 16, 'reserved': 0}
    ...
    def test_audio_converter_convert_buffer(self, source_format,
        dest_format_mono):
        ...
        try:
            ...
            output_data = capi.audio_converter_convert_buffer(converter_id,
                input_data)
    ...
    def test_audio_converter_reset(self, source_format, dest_format_mono):
        ...
        try:
            ...
            output1 = capi.audio_converter_convert_buffer(converter_id,
                input_data)
            capi.audio_converter_reset(converter_id)
            output2 = capi.audio_converter_convert_buffer(converter_id,
                input_data)
    ...
    def test_audio_converter_property_getters(self):
        ...
        assert isinstance(capi.
            get_audio_converter_property_min_input_buffer_size(), int)
        assert isinstance(capi.
            get_audio_converter_property_min_output_buffer_size(), int)
        assert isinstance(capi.
            get_audio_converter_property_max_output_packet_size(), int)
        assert isinstance(capi.
            get_audio_converter_property_max_input_packet_size(), int)
        assert isinstance(capi.
            get_audio_converter_property_sample_rate_converter_quality(), int)
        assert isinstance(capi.get_audio_converter_property_codec_quality(),
            int)
    ...
    def test_audio_converter_with_real_file(self, amen_wav_path,
        ...
        try:
            ...
            converter_id = capi.audio_converter_new(source_format,
                dest_format_mono)
            try:
                output_data = capi.audio_converter_convert_buffer(converter_id,
                    packet_data)
