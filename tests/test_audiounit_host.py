...
class TestAudioUnitPluginDiscovery:
    ...
    def test_list_all_plugins_by_category(self):
        ...
        total = sum(
            len(capi.audio_unit_find_all_components(component_type=t))
...
class TestAudioUnitParameterDiscovery:
    ...
    def audio_unit(self):
        ...
        components = capi.audio_unit_find_all_components(
            component_type="aufx", manufacturer="appl"
        )
    ...
    def test_parameter_info_with_all_parameter_ids(self, audio_unit):
        ...
        for param_id in params:
            try:
                ...
                assert info["param_id"] == param_id, (
                    f"Mismatched param_id: expected {param_id}, got {info['param_id']}"
                )
        ...
        assert len(failed_params) == 0, (
            f"Failed to get info for {len(failed_params)} parameters: {failed_params}"
        )
    ...
    def test_third_party_plugin_parameters(self):
        ...
        for comp_id in components[:20]:  # Check first 20 plugins
            ...
            try:
                ...
                if max(params[: min(5, len(params))]) > 100:
                    print(
                        "  → Plugin uses non-sequential parameter IDs (FourCC-encoded)"
                    )
                    ...
                    for param_id in params:
                        try:
                            param_info = capi.audio_unit_get_parameter_info(
                                unit_id, param_id
                            )
                    ...
                    if failed:
                        ...
                        capi.audio_component_instance_dispose(unit_id)
                        pytest.fail(
                            f"Failed to get info for {len(failed)} parameters: {failed[:3]}"
                        )

                    print(
                        f"  ✓ Successfully retrieved info for all {len(params)} parameters"
                    )
        ...
        if not third_party_tested:
            pytest.skip(
                "No third-party plugins with non-sequential parameter IDs found"
            )
...
class TestAudioUnitPresets:
    ...
    @pytest.mark.skip(
        reason="Some AudioUnits (including Apple ones) hang during initialization - run manually if needed"
    )
    def test_survey_all_presets(self):
        ...
        for category_name, category_type in categories.items():
            ...
            components = capi.audio_unit_find_all_components(
                component_type=category_type, manufacturer="appl"
            )

            if not components:
                print("  No Apple plugins found")
                continue
        ...
        if plugins_with_presets:
            ...
            sorted_plugins = sorted(
                plugins_with_presets, key=lambda x: x["preset_count"], reverse=True
            )
    ...
    @pytest.mark.slow
    @pytest.mark.skip(
        reason="Some third-party AudioUnits crash during initialization - run manually if needed"
    )
    ...
    @pytest.mark.skip(
        reason="Some AudioUnits hang during initialization - run manually if needed"
    )
