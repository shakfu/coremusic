...
class UnifiedAudioDemo:
    ...
    def demo_constants_and_utilities(self):
        ...
        try:
            ...
            self.print_success(
                f'Module loaded successfully (test error: {error_code})')
            constants = {'kAudioFormatLinearPCM': capi.
                get_audio_format_linear_pcm(),
                'kLinearPCMFormatFlagIsSignedInteger': capi.
                get_linear_pcm_format_flag_is_signed_integer(),
                'kLinearPCMFormatFlagIsPacked': capi.
                get_linear_pcm_format_flag_is_packed(),
                'kAudioFileWAVEType': capi.get_audio_file_wave_type(),
                'kAudioFileReadPermission': capi.
                get_audio_file_read_permission(),
                'kAudioFilePropertyDataFormat': capi.
                get_audio_file_property_data_format(),
                'kAudioUnitType_Output': capi.get_audio_unit_type_output(),
                'kAudioUnitSubType_DefaultOutput': capi.
                get_audio_unit_subtype_default_output(),
                'kAudioUnitManufacturer_Apple': capi.
                get_audio_unit_manufacturer_apple()}
    ...
    def demo_audio_file_operations(self):
        ...
        try:
            with wave.open(self.wav_path, 'rb') as wav:
                self.format_info = {'sample_rate': wav.getframerate(),
                    'channels': wav.getnchannels(), 'sample_width': wav.
                    getsampwidth(), 'frame_count': wav.getnframes(),
                ...
                self.print_info(
                    f"Duration: {self.format_info['duration']:.2f} seconds")
                self.print_info(
                    f'File size: {os.path.getsize(self.wav_path):,} bytes')
            audio_file_id = capi.audio_file_open_url(self.wav_path, capi.
                get_audio_file_read_permission(), capi.
                get_audio_file_wave_type())
            format_data = capi.audio_file_get_property(audio_file_id, capi.
                get_audio_file_property_data_format())
    ...
    def demo_audiounit_infrastructure(self):
        ...
        try:
            ...
            format_data = struct.pack('<dLLLLLLLL', float(self.format_info[
                'sample_rate']), capi.get_audio_format_linear_pcm(), capi.
                get_linear_pcm_format_flag_is_signed_integer() | capi.
                get_linear_pcm_format_flag_is_packed(), self.format_info[
                'channels'] * self.format_info['sample_width'], 1, self.
                format_info['channels'] * self.format_info['sample_width'],
                self.format_info['channels'], self.format_info[
                'sample_width'] * 8, 0)
            try:
                capi.audio_unit_set_property(audio_unit, capi.
                    get_audio_unit_property_stream_format(), capi.
                    get_audio_unit_scope_input(), 0, format_data)
    ...
    def demo_audioqueue_infrastructure(self):
        ...
        try:
            audio_format = {'sample_rate': float(self.format_info[
                'sample_rate']), 'format_id': capi.
                get_audio_format_linear_pcm(), 'format_flags': capi.
                get_linear_pcm_format_flag_is_signed_integer() | capi.
                get_linear_pcm_format_flag_is_packed(), 'bytes_per_packet':
                self.format_info['channels'] * self.format_info[
                'sample_width'], 'frames_per_packet': 1, 'bytes_per_frame':
                self.format_info['channels'] * self.format_info[
                'sample_width'], 'channels_per_frame': self.format_info[
                'channels'], 'bits_per_channel': self.format_info[
                'sample_width'] * 8}
    ...
    def run_comprehensive_demo(self):
        ...
        self.print_info(
            'Combining all individual demos into one comprehensive test')
        print()
        demos = [('CoreAudio Constants & Utilities', self.
            demo_constants_and_utilities), ('Audio File Operations', self.
            demo_audio_file_operations), ('AudioUnit Infrastructure', self.
            demo_audiounit_infrastructure), ('AudioQueue Infrastructure',
            self.demo_audioqueue_infrastructure), (
        ...
        successful_demos = sum(1 for success in self.demo_results.values() if
            success)
        ...
        if successful_demos == total_demos:
            self.print_success(
                'ALL DEMOS PASSED - CYCOREAUDIO IS FULLY FUNCTIONAL!')
            self.print_info(
                'The coremusic wrapper provides complete access to CoreAudio')
            self.print_info(
                'All major audio frameworks are successfully wrapped')
...
def main():
    ...
    if success:
        ...
        print(
            '   The coremusic wrapper is ready for professional audio development.'
            )
    else:
