...
class TestAudioComponentDiscovery:
    ...
    def test_audio_component_description_structure(self):
        ...
        description = {'type': capi.get_audio_unit_type_output(), 'subtype':
            ...
            capi.get_audio_unit_manufacturer_apple(), 'flags': 0,
            'flags_mask': 0}
        required_keys = ['type', 'subtype', 'manufacturer', 'flags',
            'flags_mask']
...
class TestAudioUnitInfrastructure:
    ...
    def test_audio_unit_format_configuration(self, audio_unit):
        ...
        format_data = struct.pack('<dLLLLLLLL', 44100.0, capi.
            get_audio_format_linear_pcm(), capi.
            get_linear_pcm_format_flag_is_signed_integer() | capi.
            get_linear_pcm_format_flag_is_packed(), 4, 1, 4, 2, 16, 0)
        try:
...
class TestErrorHandling:
    ...
    def test_audio_queue_with_invalid_format(self):
        ...
        invalid_format = {'sample_rate': -1, 'format_id': 0, 'format_flags':
            0, 'bytes_per_packet': 0, 'frames_per_packet': 0,
            'bytes_per_frame': 0, 'channels_per_frame': 0,
            'bits_per_channel': 0}
        with pytest.raises(Exception):
