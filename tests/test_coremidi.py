...
class TestCoreMIDIVirtualEndpoints:
    ...
    def test_midi_destination_create_dispose(self):
        ...
        destination = capi.midi_destination_create(self.client,
            'Test Virtual Destination')
        assert destination is not None
        assert isinstance(destination, int)
        assert destination > 0
...
class TestCoreMIDIProperties:
    ...
    def test_midi_object_get_integer_property(self):
        ...
        if num_devices > 0:
            device = capi.midi_get_device(0)
            try:
                unique_id = capi.midi_object_get_integer_property(device,
                    'uniqueID')
    ...
    def test_midi_object_set_string_property(self):
        ...
        try:
            capi.midi_object_set_string_property(source, 'testProperty',
                'test value')
    ...
    def test_midi_object_set_integer_property(self):
        ...
        source = capi.midi_source_create(self.client,
            'Test Integer Property Source')
        try:
            capi.midi_object_set_integer_property(source, 'testIntProperty', 42)
            value = capi.midi_object_get_integer_property(source,
                'testIntProperty')
            if value is not None:
                assert value == 42
        except RuntimeError:
...
class TestCoreMIDIIntegration:
    ...
    def test_full_midi_workflow(self):
        ...
        try:
            ...
            output_port = capi.midi_output_port_create(client,
                'Integration Output')
            virtual_source = capi.midi_source_create(client,
                'Integration Virtual Source')
            virtual_destination = capi.midi_destination_create(client,
                'Integration Virtual Destination')
            try:
                capi.midi_object_set_string_property(virtual_source,
                    'description', 'Test source')
                description = capi.midi_object_get_string_property(virtual_source
                    , 'description')
            ...
            try:
                ...
                capi.midi_send_data(output_port, virtual_destination,
                    note_data, 0)
            except RuntimeError:
                pass
    ...
    def test_midi_system_enumeration(self):
        ...
        for i in range(min(num_devices, 3)):
            ...
            for j in range(num_entities):
                entity = capi.midi_device_get_entity(device, j)
                num_entity_sources = capi.midi_entity_get_number_of_sources(
                    entity)
                num_entity_destinations = capi.midi_entity_get_number_of_destinations(entity)
                print(
...
class TestCoreMIDIMessages:
    ...
    def test_midi1_up_channel_voice_message(self):
        ...
        data1 = 60
        data2 = 127
        message = capi.midi1_up_channel_voice_message(group, status, channel,
            data1, data2)
    ...
    def test_midi1_up_sysex(self):
        ...
        byte5 = 0
        byte6 = 0
        message = capi.midi1_up_sysex(group, status, bytes_used, byte1, byte2,
    ...
    def test_midi2_channel_voice_message(self):
        ...
        index = 15360
        value = 4294901760
        message = capi.midi2_channel_voice_message(group, status, channel,
            index, value)
        assert isinstance(message, tuple)
        assert len(message) == 2
        assert isinstance(message[0], int)
    ...
    def test_midi2_note_on(self):
        ...
        attribute_data = 0
        velocity = 65535
        message = capi.midi2_note_on(group, channel, note_number,
            attribute_type, attribute_data, velocity)
        assert isinstance(message, tuple)
        assert len(message) == 2
        assert isinstance(message[0], int)
        assert isinstance(message[1], int)
    ...
    def test_midi2_note_off(self):
        ...
        attribute_data = 0
        velocity = 32768
        message = capi.midi2_note_off(group, channel, note_number,
            attribute_type, attribute_data, velocity)
        assert isinstance(message, tuple)
        assert len(message) == 2
        assert isinstance(message[0], int)
    ...
    def test_midi2_program_change(self):
        ...
        bank_msb = 0
        bank_lsb = 0
        message = capi.midi2_program_change(group, channel, bank_is_valid,
    ...
    def test_midi_message_parameter_validation(self):
        ...
        max_note = 127
        max_velocity = 127
        message = capi.midi1_up_note_on(max_group, max_channel, max_note,
            max_velocity)
        assert isinstance(message, int)
    ...
    def test_midi_message_consistency(self):
        ...
        attr_data = 0
        velocity16 = 51200
        msg2_1 = capi.midi2_note_on(group, channel, note, attr_type,
            attr_data, velocity16)
        msg2_2 = capi.midi2_note_on(group, channel, note, attr_type,
            attr_data, velocity16)
...
class TestCoreMIDISetup:
    ...
    def test_midi_external_device_create(self):
        ...
        try:
            device = capi.midi_external_device_create('Test External Device',
                'Test Manufacturer', 'Test Model')
            assert isinstance(device, int)
            assert device > 0
    ...
    def test_midi_device_add_entity_basic(self):
        ...
        device = capi.midi_get_device(0)
        try:
            entity = capi.midi_device_add_entity(device, 'Test Entity', 
                True, 1, 1)
            assert isinstance(entity, int)
            assert entity > 0
            try:
    ...
    def test_midi_device_new_entity_basic(self):
        ...
        device = capi.midi_get_device(0)
        try:
            entity = capi.midi_device_new_entity(device,
                'Test MIDI 1.0 Entity', 1, True, 1, 1)
            assert isinstance(entity, int)
            assert entity > 0
            try:
                capi.midi_device_remove_entity(device, entity)
        ...
        try:
            entity = capi.midi_device_new_entity(device,
                'Test MIDI 2.0 Entity', 2, False, 2, 2)
            assert isinstance(entity, int)
            assert entity > 0
            try:
                capi.midi_device_remove_entity(device, entity)
    ...
    def test_midi_setup_integration_workflow(self):
        ...
        try:
            device = capi.midi_external_device_create('Integration Test Device',
                'Test Company', 'Test Model v1.0')
            assert isinstance(device, int)
            assert device > 0
            try:
                capi.midi_setup_add_external_device(device)
                try:
                    entity = capi.midi_device_add_entity(device,
                        'Test Entity', False, 1, 1)
                    try:
            ...
            pytest.skip(
                'External device creation not supported in this environment')
...
class TestCoreMIDIDriver:
    ...
    def test_midi_device_create_basic(self):
        ...
        try:
            device = capi.midi_device_create('Driver Test Device',
                'Test Driver Manufacturer', 'Test Driver Model')
            assert isinstance(device, int)
            assert device > 0
            try:
    ...
    def test_midi_endpoint_ref_cons_basic(self):
        ...
        ref2_value = 67890
        try:
            result = capi.midi_endpoint_set_ref_cons(source, ref1_value,
                ref2_value)
            assert isinstance(result, int)
            assert result == 0
            ref1_retrieved, ref2_retrieved = capi.midi_endpoint_get_ref_cons(
                source)
    ...
    def test_midi_device_list_operations_basic(self):
        ...
        try:
            num_devices = capi.midi_device_list_get_number_of_devices(
                invalid_dev_list)
            assert isinstance(num_devices, int)
            assert num_devices >= 0
        except Exception:
    ...
    def test_midi_driver_integration_workflow(self):
        ...
        try:
            device = capi.midi_device_create('Integration Driver Device',
                'Integration Test Company', 'Integration Test Model')
            assert isinstance(device, int)
            assert device > 0
            try:
                capi.midi_device_dispose(device)
...
class TestCoreMIDIThruConnection:
    ...
    def test_midi_thru_connection_create_persistent(self):
        ...
        try:
            ...
            pytest.skip(
                f'Persistent thru connection creation failed (expected): {e}')
    ...
    def test_midi_thru_connection_create_with_params(self):
        ...
        try:
            ...
            connection = capi.midi_thru_connection_create(persistent_owner_id
                =self.test_owner_id, connection_params=params)
            assert isinstance(connection, int)
            assert connection > 0
            self.test_connections.append(connection)
            ...
            pytest.skip(
                f'Thru connection with custom params failed (expected): {e}')
    ...
    def test_midi_thru_connection_with_endpoints(self):
        ...
        try:
            ...
            if num_destinations > 0:
                ...
                assert retrieved_params['destinations'][0]['endpointRef'
                    ] == dest
            capi.midi_thru_connection_dispose(connection)
            self.test_connections.remove(connection)
        except RuntimeError as e:
