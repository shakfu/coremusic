...
class TestCoreMIDIConstants:
    ...
    def test_midi_error_constants(self):
        ...
        assert ca.get_midi_error_invalid_client() == -10830
        assert ca.get_midi_error_invalid_port() == -10831
        assert ca.get_midi_error_wrong_endpoint_type() == -10832
        assert ca.get_midi_error_no_connection() == -10833
        assert ca.get_midi_error_unknown_endpoint() == -10834
        assert ca.get_midi_error_unknown_property() == -10835
        assert ca.get_midi_error_wrong_property_type() == -10836
        assert ca.get_midi_error_no_current_setup() == -10837
        assert ca.get_midi_error_message_send_err() == -10838
        assert ca.get_midi_error_server_start_err() == -10839
        assert ca.get_midi_error_setup_format_err() == -10840
        assert ca.get_midi_error_wrong_thread() == -10841
        assert ca.get_midi_error_object_not_found() == -10842
        assert ca.get_midi_error_id_not_unique() == -10843
        assert ca.get_midi_error_not_permitted() == -10844
        assert ca.get_midi_error_unknown_error() == -10845

    def test_midi_object_type_constants(self):
        """Test MIDI object type constants"""
        assert ca.get_midi_object_type_other() == -1
        assert ca.get_midi_object_type_device() == 0
        assert ca.get_midi_object_type_entity() == 1
        assert ca.get_midi_object_type_source() == 2
        assert ca.get_midi_object_type_destination() == 3
        assert ca.get_midi_object_type_external_device() == 0x10
        assert ca.get_midi_object_type_external_entity() == 0x11
        assert ca.get_midi_object_type_external_source() == 0x12
        assert ca.get_midi_object_type_external_destination() == 0x13

    def test_midi_protocol_constants(self):
        """Test MIDI protocol constants"""
        assert ca.get_midi_protocol_1_0() == 1
        assert ca.get_midi_protocol_2_0() == 2
...
class TestCoreMIDIClient:
    ...
    def test_midi_client_create_dispose(self):
        ...
        client = ca.midi_client_create("Test Client")
        assert client is not None
        assert isinstance(client, int)
        assert client > 0

        # Dispose the client
        ca.midi_client_dispose(client)
...
class TestCoreMIDIPorts:
    ...
    def setup_method(self):
        ...
        self.client = ca.midi_client_create("Test Port Client")

    def teardown_method(self):
        """Clean up test client"""
        try:
            if hasattr(self, 'client') and self.client:
                ca.midi_client_dispose(self.client)
                self.client = None
        except Exception:
            pass
    ...
    def test_midi_input_port_create_dispose(self):
        ...
        port = ca.midi_input_port_create(self.client, "Test Input Port")
        assert port is not None
        assert isinstance(port, int)
        assert port > 0

        ca.midi_port_dispose(port)

    def test_midi_output_port_create_dispose(self):
        """Test MIDI output port creation and disposal"""
        port = ca.midi_output_port_create(self.client, "Test Output Port")
        assert port is not None
        assert isinstance(port, int)
        assert port > 0
    ...
    def test_midi_port_create_invalid_client(self):
        ...
        with pytest.raises(RuntimeError):
            ca.midi_input_port_create(0, "Invalid Client Port")
...
class TestCoreMIDIDevices:
    ...
    def test_midi_get_number_of_destinations(self):
        ...
        num_destinations = ca.midi_get_number_of_destinations()
    ...
    def test_midi_device_enumeration(self):
        ...
        for i in range(num_devices):
            device = ca.midi_get_device(i)
            assert device is not None
            assert isinstance(device, int)
            assert device > 0

            # Test getting entities for this device
            num_entities = ca.midi_device_get_number_of_entities(device)
            assert isinstance(num_entities, int)
            assert num_entities >= 0

            for j in range(num_entities):
                entity = ca.midi_device_get_entity(device, j)
    ...
    def test_midi_destination_enumeration(self):
        ...
        for i in range(min(num_destinations, 5)):  # Test first 5 destinations max
            destination = ca.midi_get_destination(i)
...
class TestCoreMIDIVirtualEndpoints:
    ...
    def setup_method(self):
        ...
        self.client = ca.midi_client_create("Test Virtual Client")
    ...
    def test_midi_source_create_dispose(self):
        ...
        source = ca.midi_source_create(self.client, "Test Virtual Source")
        assert source is not None
        assert isinstance(source, int)
        assert source > 0

        ca.midi_endpoint_dispose(source)

    def test_midi_destination_create_dispose(self):
        """Test virtual MIDI destination creation and disposal"""
        destination = ca.midi_destination_create(self.client, "Test Virtual Destination")
        assert destination is not None
        assert isinstance(destination, int)
        assert destination > 0

        ca.midi_endpoint_dispose(destination)
...
class TestCoreMIDIProperties:
    ...
    def setup_method(self):
        ...
        self.client = ca.midi_client_create("Test Property Client")
    ...
    def test_midi_object_get_string_property(self):
        ...
        num_devices = ca.midi_get_number_of_devices()
        if num_devices > 0:
            device = ca.midi_get_device(0)
            try:
                name = ca.midi_object_get_string_property(device, "name")
                if name is not None:
    ...
    def test_midi_object_get_integer_property(self):
        ...
        if num_devices > 0:
            device = ca.midi_get_device(0)
            try:
                unique_id = ca.midi_object_get_integer_property(device, "uniqueID")
    ...
    def test_midi_object_set_string_property(self):
        ...
        source = ca.midi_source_create(self.client, "Test Property Source")

        try:
            # Try to set a custom property
            ca.midi_object_set_string_property(source, "testProperty", "test value")

            # Try to get it back
            value = ca.midi_object_get_string_property(source, "testProperty")
    ...
    def test_midi_object_set_integer_property(self):
        ...
        # Create a virtual source to test property setting
        source = ca.midi_source_create(self.client, "Test Integer Property Source")

        try:
            # Try to set a custom integer property
            ca.midi_object_set_integer_property(source, "testIntProperty", 42)

            # Try to get it back
            value = ca.midi_object_get_integer_property(source, "testIntProperty")
            if value is not None:
                assert value == 42
        except RuntimeError:
            # Some properties might be read-only, which is okay
...
class TestCoreMIDIData:
    ...
    def test_midi_send_data_large(self):
        ...
        with pytest.raises(ValueError):
            ca.midi_send_data(1, 1, large_data, 0)
    ...
    def test_midi_send_data_basic_success(self):
        ...
        result = ca.midi_send_data(1, 1, note_on_data, 0)
...
class TestCoreMIDIIntegration:
    ...
    def test_full_midi_workflow(self):
        ...
        # Create client
        client = ca.midi_client_create("Integration Test Client")

        try:
            # Create ports
            input_port = ca.midi_input_port_create(client, "Integration Input")
            output_port = ca.midi_output_port_create(client, "Integration Output")

            # Create virtual endpoints
            virtual_source = ca.midi_source_create(client, "Integration Virtual Source")
            virtual_destination = ca.midi_destination_create(client, "Integration Virtual Destination")

            # Test property operations
            try:
                ca.midi_object_set_string_property(virtual_source, "description", "Test source")
                description = ca.midi_object_get_string_property(virtual_source, "description")
            ...
            try:
                ...
                ca.midi_send_data(output_port, virtual_destination, note_data, 0)
            except RuntimeError:
                # Data sending might fail without proper connections
                pass

            # Clean up
            ca.midi_endpoint_dispose(virtual_destination)
            ca.midi_endpoint_dispose(virtual_source)
            ca.midi_port_dispose(output_port)
            ca.midi_port_dispose(input_port)
    ...
    def test_midi_system_enumeration(self):
        ...
        for i in range(min(num_devices, 3)):  # Test first 3 devices max
            ...
            for j in range(num_entities):
                entity = ca.midi_device_get_entity(device, j)
                num_entity_sources = ca.midi_entity_get_number_of_sources(entity)
                num_entity_destinations = ca.midi_entity_get_number_of_destinations(entity)
                ...
                for k in range(min(num_entity_sources, 2)):
                    source = ca.midi_entity_get_source(entity, k)
                    assert source > 0

                for k in range(min(num_entity_destinations, 2)):
                    destination = ca.midi_entity_get_destination(entity, k)
                    assert destination > 0
